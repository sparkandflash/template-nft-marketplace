...
export default function MyProfile() {
  ...
  async function checkNetwork() {
    ...
    if (chainId != 5) {
      addToast({
        title: "Alert!.",
        description: "you are on wrong network, please connect to georli network",
        status: "warning",
        duration: 9000,
        isClosable: true,
      })
      ...
      console.log("connected to georli network, chainid : 5")
    }
          ...
          <Box w='100%' borderRadius='lg' padding={5} borderWidth='1px'>
            <Center>
              <VStack padding={3}>
                ...
                <Tag visibility={user.role}>creator</Tag>
            ...
            <Center>
              <VStack>
                <Heading as='h5' size='sm'>
                  ...
                  <Button colorScheme="blue" isDisabled='true'>add Item</Button>
           {/*     <Button size='sm' disabled={!user.role} colorScheme="blue" onClick={() => router.push('/citizen')}>add item</Button> */}
                <Text color='gray.500' >Ipfs service is being set up, mint funtion is currently disabled.</Text>
                <Box>
                  <Text p={2}>Items owned: {nfts.length} Items bought: {nfts.length}</Text>
                </Box>

                { (loadingState === 'loaded' && !nfts.length)?  <Text>No Items available</Text>  : <Box rounded={6} width='max-content' border='1px' borderColor='gray.300' padding='15px'>
